...
Версия 6.0 - Добавлена комплексная вкладка "Анимация процесса"
Автор: Жданов А. О.
"""
...
# Глобальные переменные, будут инициализированы в _load_materials
...
class WeldingApp(tk.Tk):
    ...
    def _map_color(self, color_str):
        if not color_str:
            return color_str
        norm = self._normalize_color(color_str)
        ...
        for key, val in old_theme.items():
            if self._normalize_color(val) == norm:
            ...
            for prop in color_props:
                try:
                    if prop in widget.keys():
                        ...
                        new_val = self._map_color(old_val)
    ...
    def _update_highlights(self):
        if hasattr(self, 'tmax_frame'):
            self.tmax_frame.configure(bg=COLORS["accent2"])
            self.tmax_label.configure(bg=COLORS["accent2"], fg="#ffffff")
        if hasattr(self, 'frm6_highlight'):
            self.frm6_highlight.configure(highlightbackground=COLORS["accent2"])
            self.hdr6.configure(bg=COLORS["accent2"])
            self.hdr6_label.configure(bg=COLORS["accent2"])
        if hasattr(self, 'cycle_hdr'):
            self.cycle_hdr.configure(bg=COLORS["accent2"])
            self.cycle_hdr_label.configure(bg=COLORS["accent2"])
        if hasattr(self, 'rec_hdr'):
            self.rec_hdr.configure(bg=COLORS["accent2"])
            self.rec_hdr_label.configure(bg=COLORS["accent2"])

    def _update_ttk_style(self):
        style = ttk.Style()
        style.theme_use("clam")
        style.configure("TNotebook", background=COLORS["bg"], borderwidth=0)
        style.configure("TNotebook.Tab", background=COLORS["bg3"], foreground=COLORS["text2"], 
        ...
        style.configure("TScrollbar", background=COLORS["bg3"], troughcolor=COLORS["bg2"], bordercolor=COLORS["border"], arrowcolor=COLORS["text"])
        style.map("TScrollbar", background=[("active", COLORS["accent"]), ("!active", COLORS["bg3"])])
        style.configure("TCombobox", fieldbackground=COLORS["bg3"], background=COLORS["bg3"], foreground=COLORS["text"], bordercolor=COLORS["border"], arrowcolor=COLORS["text"])
        style.map("TCombobox", fieldbackground=[("readonly", COLORS["bg3"])])
    ...
    def toggle_theme(self):
        ...
        for fig_attr in ["fig_cycle", "fig_anim", "fig_cooling", "fig_components", "fig_propagation", 
            ...
            if hasattr(self, fig_attr):
                getattr(self, fig_attr).patch.set_facecolor(COLORS["bg2"])
    ...
    def _add_highlight_header(self, parent, text, attr_name):
        hdr = tk.Frame(parent, bg=COLORS["accent2"], height=32)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        lbl = tk.Label(hdr, text=f"  {text} ", font=("Consolas", 12, "bold"), fg="#ffffff", bg=COLORS["accent2"], anchor="w")
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr, text="✨ МОДЕЛИРОВАНИЕ ТЕПЛОВОГО ПРОЦЕССА СВАРКИ ",
                 font=("Consolas", 14, "bold"), fg=COLORS["accent"], bg=COLORS["header"]).pack(side="left", padx=10, pady=14)
        
        tk.Label(hdr, text="| Автор: Жданов А. О.", font=FONT_UI, fg=COLORS["accent2"], bg=COLORS["header"]).pack(side="left", padx=5)
        
        disp_frame_hdr = tk.Frame(hdr, bg=COLORS["header"])
        disp_frame_hdr.pack(side="left", padx=30)
        tk.Label(disp_frame_hdr, text="Отображать:", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["header"]).pack(side="left", padx=5)
        tk.Radiobutton(disp_frame_hdr, text="Равномерный", variable=self.var_display_mode, value="rect", font=FONT_UI, fg=COLORS["text"], bg=COLORS["header"], selectcolor=COLORS["bg3"], command=self._update_plots_from_results).pack(side="left", padx=5)
        tk.Radiobutton(disp_frame_hdr, text="Гауссов", variable=self.var_display_mode, value="gauss", font=FONT_UI, fg=COLORS["text"], bg=COLORS["header"], selectcolor=COLORS["bg3"], command=self._update_plots_from_results).pack(side="left", padx=5)

        styled_button(hdr, "🌓 Тема", self.toggle_theme, color=COLORS["bg3"]).pack(side="right", padx=20)
        
        status_bar = tk.Frame(self, bg=COLORS["bg3"], height=40)
        status_bar.pack(fill="x", side="bottom")
        status_bar.pack_propagate(False)
        
        self.status_var = tk.StringVar(value="Готов к расчёту")
        tk.Label(status_bar, textvariable=self.status_var, font=FONT_UI_BOLD, fg=COLORS["text2"], bg=COLORS["bg3"], anchor="w").pack(side="left", padx=12, pady=6)
        
        self.tmax_frame = tk.Frame(status_bar, bg=COLORS["accent2"], padx=15, pady=4)
        self.tmax_frame.pack(side="right", fill="y", padx=5, pady=5)
        self.tmax_label = tk.Label(self.tmax_frame, text=" T_max = --- °C  ", font=("Consolas", 12, "bold"), fg="#ffffff", bg=COLORS["accent2"])
    ...
    def _build_params_panel(self, parent):
        ...
        p = scroll_frame
        tk.Label(p, text=" ПАРАМЕТРЫ МОДЕЛИ ", font=("Consolas", 12, "bold"), fg=COLORS["accent"], bg=COLORS["bg"]).pack(anchor="w", padx=10, pady=(12,4))
        ...
        btn_mat_frame = tk.Frame(inn_mat, bg=COLORS["bg2"])
        btn_mat_frame.pack(fill="x", pady=2)
        
        self.btn_apply_mat = styled_button(btn_mat_frame, "↻ Применить", self._apply_material_props, color=COLORS["accent3"])
        self.btn_apply_mat.pack(side="left", expand=True, fill="x", padx=2)
        
        self.btn_save_mat = styled_button(btn_mat_frame, "💾 Сохранить", self._save_custom_material, color=COLORS["bg3"])
        self.btn_save_mat.pack(side="left", expand=True, fill="x", padx=2)
        
        self.btn_delete_mat = styled_button(btn_mat_frame, " 🗑 ", self._delete_custom_material, color=COLORS["accent4"])
        self.btn_delete_mat.pack(side="left", expand=True, fill="x", padx=2)
        
        btn_db_frame = tk.Frame(inn_mat, bg=COLORS["bg2"])
        btn_db_frame.pack(fill="x", pady=2)
        styled_button(btn_db_frame, "📤 Экспорт базы", self._export_builtin_materials, color=COLORS["bg3"]).pack(side="left", expand=True, fill="x", padx=2)
        styled_button(btn_db_frame, "📥 Импорт базы", self._import_builtin_materials, color=COLORS["bg3"]).pack(side="left", expand=True, fill="x", padx=2)
        
        self.mat_desc_label = tk.Label(inn_mat, text="", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"], wraplength=320, justify="left")
        ...
        tk.Label(inn2, text="Форма источника (расчет):", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"]).pack(anchor="w", pady=(10,0))
        ...
        tk.Label(inn3, text="(< 0 = охлаждение, > 0 = подогрев)", font=FONT_UI, fg=COLORS["warning"], bg=COLORS["bg2"]).pack(anchor="w")
        tk.Label(inn3, text="⚠ Для эффекта используйте |q| > 100", font=FONT_UI, fg=COLORS["accent2"], bg=COLORS["bg2"]).pack(anchor="w")
        ...
        self.btn_calculate = styled_button(btn_frame, "▶ РАССЧИТАТЬ", self._run_all, color=COLORS["accent"])
        ...
        self.frm6_highlight = tk.Frame(p, bg=COLORS["bg2"], highlightthickness=2, highlightbackground=COLORS["accent2"])
        self.frm6_highlight.pack(fill="x", padx=8, pady=4)
        self.hdr6 = tk.Frame(self.frm6_highlight, bg=COLORS["accent2"])
        self.hdr6.pack(fill="x")
        self.hdr6_label = tk.Label(self.hdr6, text="  📊 Анализ результата ", font=FONT_UI_BOLD, fg="#ffffff", bg=COLORS["accent2"], pady=6)
        self.hdr6_label.pack(side="left")
        inn6 = tk.Frame(self.frm6_highlight, bg=COLORS["bg2"])
        inn6.pack(fill="both", expand=True, padx=12, pady=8)
        self.reco_text = tk.Text(inn6, height=8, width=36, font=FONT_UI, bg=COLORS["bg3"], fg=COLORS["text2"], relief="flat", state="disabled", wrap="word")
    ...
    def _param_row(self, parent, label, var):
        ...
        tk.Label(row, text=label, font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"], width=28, anchor="w").pack(side="left")
    ...
    def _on_mat_source_changed(self):
        if self.var_mat_source.get() == "builtin":
            if not BUILTIN_MATERIALS:
                self.material_combo["values"] = ["(нет материалов)"]
                self.var_material.set("(нет материалов)")
            else:
                self.material_combo["values"] = BUILTIN_MATERIALS
                if self.var_material.get() not in BUILTIN_MATERIALS:
                    self.var_material.set(BUILTIN_MATERIALS[0])
        else:
            custom_list = [m for m in MATERIALS_DB.keys() if m not in BUILTIN_MATERIALS]
            if not custom_list:
    ...
    def _update_mat_buttons_state(self):
        ...
        is_builtin = mat_name in BUILTIN_MATERIALS
    ...
    def _apply_material_props(self):
        mat_name = self.var_material.get()
        if mat_name not in MATERIALS_DB: return
        props = MATERIALS_DB[mat_name]
    ...
    def _save_custom_material(self):
        ...
        dialog.grab_set()
        tk.Label(dialog, text="Название: ", fg=COLORS["text"], bg=COLORS["bg"], font=FONT_UI_BOLD).pack(pady=10)
        name_entry = tk.Entry(dialog, width=40, font=FONT_UI, bg=COLORS["bg3"], fg=COLORS["text"])
        name_entry.pack(pady=5)
        tk.Label(dialog, text="Описание: ", fg=COLORS["text2"], bg=COLORS["bg"], font=FONT_UI).pack(pady=(15,5))
        desc_entry = tk.Entry(dialog, width=40, font=FONT_UI, bg=COLORS["bg3"], fg=COLORS["text2"])
    ...
    def _build_tabs(self, parent):
        self._update_ttk_style()
        btn_frame = tk.Frame(parent, bg=COLORS["bg3"], height=32)
        ...
        tk.Label(btn_frame, text="  Навигация", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg3"]).pack(side="left", padx=10)
    ...
    def _build_tab_cycle(self, parent):
        ...
        toolbar = tk.Frame(frame, bg=COLORS["bg3"])
        toolbar.pack(fill="x")
    ...
    def _build_tab_animation(self, parent):
        ...
        self.btn_anim_play = styled_button(ctrl, "▶ Play", self._toggle_anim, color=COLORS["accent3"])
        self.btn_anim_play.pack(side="left", padx=5)
        
        styled_button(ctrl, "⏹ Reset", self._reset_anim, color=COLORS["bg3"]).pack(side="left", padx=5)
        
        tk.Label(ctrl, text="Время:", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"]).pack(side="left", padx=(20, 5))
        
        self.anim_slider = tk.Scale(ctrl, from_=0, to=100, orient="horizontal", length=300, 
                                    bg=COLORS["bg3"], fg=COLORS["text"], troughcolor=COLORS["bg2"],
                                    highlightthickness=0, command=self._on_anim_slider_change)
        self.anim_slider.pack(side="left", padx=5)
        
        tk.Label(ctrl, text="Скорость:", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"]).pack(side="left", padx=(20, 5))
        ...
        self.anim_info_label = tk.Label(ctrl, text="t = 0.00 с | T_max = 0 °C", font=FONT_UI_BOLD, fg=COLORS["accent"], bg=COLORS["bg2"])
        ...
        self.fig_anim = Figure(figsize=(10, 8), facecolor=COLORS["bg2"])
    ...
    def _toggle_anim(self):
        ...
        if self.anim_playing:
            self.btn_anim_play.config(text="⏸ Pause", color=COLORS["accent4"])
            if self.anim_frame_idx >= len(self._t_arr) - 1:
                self.anim_frame_idx = 0
            self._run_anim_loop()
        else:
            self.btn_anim_play.config(text="▶ Play", color=COLORS["accent3"])
    ...
    def _draw_anim_frame(self, idx):
        ...
        ax1.plot(self._y_arr, T_profile, color=COLORS["accent"], lw=2.5)
        ax1.fill_between(self._y_arr, 0, T_profile, alpha=0.2, color=COLORS["accent"])
        
        ax1.axhline(p["T_melt"], color=COLORS["melt"], lw=1.5, ls="--", alpha=0.7, label=f"T_пл={p['T_melt']:.0f}°C")
        if p["T_harden"] > 0:
            ax1.axhline(p["T_harden"], color=COLORS["warning"], lw=1.5, ls="--", alpha=0.7, label=f"T_зак={p['T_harden']:.0f}°C")
            
        if p["src_type"] == "rect":
            ax1.axvspan(self.var_y1.get(), self.var_y2.get(), alpha=0.15, color=COLORS["accent2"], label="Зона источника")
        else:
            ax1.axvline(p["l"]/2, color=COLORS["accent2"], lw=2, ls=":", label="Центр Гаусса")
            
        ax1.set_ylim(0, max(T_max_curr * 1.1, p["T_melt"] * 1.1))
        ax1.legend(fontsize=FONT_PLOT_LABEL-1, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"], loc='upper right')
        ...
        l = p["l"]
        plate_height = 2.0
        plate = Rectangle((-l/2, 0), l, plate_height, color=COLORS["bg3"], edgecolor=COLORS["text"], lw=2)
        ax2.add_patch(plate)
        ...
        if np.any(melted):
            ...
            weld_rect = Rectangle((y_left - l/2, 0), w_weld, plate_height, color=COLORS["melt"], alpha=0.8)
        ...
        if p["T_harden"] > 0:
            ...
            if np.any(hardened):
                ...
                haz_rect = Rectangle((y_left_h - l/2, 0), w_haz, plate_height, color=COLORS["warning"], alpha=0.5)
        ...
        if not hasattr(self, '_anim_im') or self._anim_im is None:
            ...
            ax3._anim_cbar.set_label("T, °C", color=COLORS["text2"])
        else:
    ...
    def _build_tab_fourier(self, parent):
        frame = tk.Frame(parent, bg=COLORS["bg2"])
        frame.pack(fill="both", expand=True, padx=4, pady=2)
        ctrl = tk.Frame(frame, bg=COLORS["bg2"])
        ctrl.pack(fill="x", padx=8, pady=2)
        tk.Label(ctrl, text="N гармоник: ", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"]).pack(side="left", padx=6)
    ...
    def _build_tab_heatmap(self, parent):
        frame = tk.Frame(parent, bg=COLORS["bg2"])
        frame.pack(fill="both", expand=True, padx=4, pady=4)
        fig = Figure(figsize=(10, 9), facecolor=COLORS["bg2"])
    ...
    def _build_tab_metallurgy(self, parent):
        ...
        self.lbl_t85 = tk.Label(info_frame, text="t₈/₅: — с ", font=FONT_UI_BOLD, fg=COLORS["accent"], bg=COLORS["bg3"])
        self.lbl_t85.pack(side="left", padx=10, pady=5)
        self.lbl_weld_width = tk.Label(info_frame, text="Ширина шва: — см ", font=FONT_UI_BOLD, fg=COLORS["accent2"], bg=COLORS["bg3"])
        self.lbl_weld_width.pack(side="left", padx=10, pady=5)
        self.lbl_haz_width = tk.Label(info_frame, text="Ширина ЗТВ: — см ", font=FONT_UI_BOLD, fg=COLORS["accent3"], bg=COLORS["bg3"])
        self.lbl_haz_width.pack(side="left", padx=10, pady=5)
        self.lbl_structure = tk.Label(info_frame, text="Структура: — ", font=FONT_UI_BOLD, fg=COLORS["text"], bg=COLORS["bg3"])
        self.lbl_structure.pack(side="left", padx=10, pady=5)
        info = tk.Label(bottom_frame, text="ⓘ t₈/₅ (800→500°C) — стандарт для сталей. Для Al/Cu не применим. ", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"])
    ...
    def _build_tab_physical(self, parent):
        ...
        info = tk.Label(frame, text="Схематическое поперечное сечение сварного соединения (масштаб по Y увеличен для наглядности)", 
                        font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"])
        info.pack(pady=5)
    ...
    def _build_tab_propagation(self, parent):
        ...
        tk.Label(ctrl, text="Время, с: ", font=FONT_UI, fg=COLORS["text2"], bg=COLORS["bg2"]).pack(side="left", padx=6)
        ...
        self.anim_time_label = tk.Label(ctrl, text=" ", font=FONT_UI_BOLD, fg=COLORS["accent3"], bg=COLORS["bg2"])
    ...
    def _build_tab_recommendations(self, parent):
        ...
        self.rec_text_widget = tk.Text(frame, wrap=tk.WORD, font=FONT_UI, bg=COLORS["bg3"], fg=COLORS["text"], relief="flat", padx=20, pady=20)
    ...
    def _build_tab_theory(self, parent):
        ...
        text_widget = tk.Text(text_frame, wrap=tk.WORD, font=FONT_UI, bg=COLORS["bg3"], fg=COLORS["text"], relief="flat", padx=20, pady=20)
...
"""
    ...
    def _plot_cycle(self, t_arr, T_cycle, p):
        ...
        ax.plot(t_arr, T_cycle, color=COLORS["accent"], lw=2.5, label="T(y_A, t)")
        T_melt = p["T_melt"]
        T_harden = p["T_harden"]
        if T_cycle.max() > T_melt:
            ax.axhline(T_melt, color=COLORS["melt"], lw=1.5, ls="--", label=f"T_пл={T_melt:.0f}°C")
        ...
        if T_harden > 0 and T_cycle.max() > T_harden:
            ax.axhline(T_harden, color=COLORS["warning"], lw=1.5, ls="--", label=f"T_зак={T_harden:.0f}°C")
        ax.axvline(p["t_source_end"], color=COLORS["accent3"], lw=1, ls=":", label=f"Конец источника")
        idx_max = np.argmax(T_cycle)
        ax.scatter(t_arr[idx_max], T_cycle[idx_max], color=COLORS["accent2"], zorder=5, s=80)
        ax.annotate(f"  T_max={T_cycle[idx_max]:.0f}°C", xy=(t_arr[idx_max], T_cycle[idx_max]), color=COLORS["accent2"], fontsize=FONT_PLOT_LABEL, fontweight="bold", xytext=(6, -20), textcoords="offset points")
        ax.legend(fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"])
    ...
    def _plot_cooling_rate(self, t_arr, T_cycle, p):
        ...
        ax.axhline(-30, color=COLORS["warning"], lw=1.5, ls="--", label="Критическая (30°C/с)")
        ax.axhline(-50, color=COLORS["melt"], lw=1.5, ls="--", label="Закалка (50°C/с)")
        ax.axhline(0, color=COLORS["text2"], lw=0.5, ls="-")
        idx_cool = np.argmin(dTdt)
        ax.scatter(t_arr[idx_cool], dTdt[idx_cool], color=COLORS["accent2"], zorder=5, s=80)
        ax.annotate(f"  max={dTdt[idx_cool]:.1f}°C/с", xy=(t_arr[idx_cool], dTdt[idx_cool]), color=COLORS["accent2"], fontsize=FONT_PLOT_LABEL, fontweight="bold")
        ax.legend(fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"])
        ax.grid(True, color=COLORS["border"], lw=0.5, alpha=0.6)
    ...
    def _plot_components(self, y_arr, t_arr, p):
        ...
        ax1.legend(fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"])
        ax1.grid(True, color=COLORS["border"], lw=0.5, alpha=0.6)
        ax2 = self.ax_comp_A
        ax2.clear()
    ...
    def _update_propagation(self, *_):
        ...
        ax.annotate(f"  T_max={T_profile[idx_max]:.0f}°C\n  y={self._y_arr[idx_max]:.1f} см", xy=(self._y_arr[idx_max], T_profile[idx_max]), color=COLORS["accent2"], fontsize=FONT_PLOT_LABEL, fontweight="bold", xytext=(10, 10), textcoords="offset points", bbox=dict(boxstyle="round,pad=0.3", facecolor=COLORS["bg3"], edgecolor=COLORS["accent2"]))
        ax.legend(fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"])
        ax.grid(True, color=COLORS["border"], lw=0.5, alpha=0.6)
    ...
    def _plot_phases(self, t_arr, T_cycle, p):
        ...
        ax.legend(handles=legend_elements, fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"], loc='upper right')
        ax.grid(True, color=COLORS["border"], lw=0.5, alpha=0.6)
    ...
    def _plot_metallurgy(self, t_arr, T_cycle, y_arr, T_field, p):
        ...
        valid = t_cool > 0
        if np.any(valid):
            ax.plot(t_cool[valid], T_cool[valid], color=COLORS["accent"], lw=2.5, label="Термический цикл")
    ...
    def _plot_physical(self, res, p):
        ...
        l = p["l"]
        plate_height = 2.0 
        plate = Rectangle((-l/2, 0), l, plate_height, color=COLORS["bg3"], edgecolor=COLORS["text"], lw=2, label="Основной металл")
        ax.add_patch(plate)
        if haz_w > 0:
            haz = Rectangle((-haz_w/2, 0), haz_w, plate_height, color=COLORS["warning"], alpha=0.5, label=f"ЗТВ ({haz_w:.3f} см)")
            ax.add_patch(haz)
        if weld_w > 0:
            weld = Rectangle((-weld_w/2, 0), weld_w, plate_height, color=COLORS["melt"], alpha=0.8, label=f"Шов ({weld_w:.3f} см)")
            ax.add_patch(weld)
        ax.axvline(0, color=COLORS["accent"], lw=2, ls="--", label="Центр источника")
        ...
        ax.legend(loc="upper right", fontsize=FONT_PLOT_LABEL, facecolor=COLORS["bg3"], labelcolor=COLORS["text"], edgecolor=COLORS["border"])
    ...
    def _plot_heatmap(self, y_arr, t_arr, T_field, p):
        ...
        self.cbar_heatmap.set_label("T, °C", color=COLORS["text2"], fontsize=FONT_PLOT_LABEL)
        ...
        try: ax.contour(t_arr, y_arr, T_plot, levels=[p["T_melt"]], colors=[COLORS["melt"]], linewidths=1.5, linestyles="--")
        except Exception: pass
    ...
    def _plot_gradient(self, y_arr, t_arr, T_field, p):
        ...
        self.cbar_gradient.set_label("|∇T|, °C/см", color=COLORS["text2"], fontsize=FONT_PLOT_LABEL)
    ...
    def _plot_3d(self, y_arr, t_arr, T_field):
        ...
        ax.set_xlabel("t, с", color=COLORS["text2"], fontsize=FONT_PLOT_LABEL, labelpad=6)
        ax.set_ylabel("y, см", color=COLORS["text2"], fontsize=FONT_PLOT_LABEL, labelpad=6)
        ax.set_zlabel("T, °C", color=COLORS["text2"], fontsize=FONT_PLOT_LABEL, labelpad=6)
        ax.set_title("3D: T(y, t)", color=COLORS["text"], fontsize=FONT_PLOT_TITLE, fontweight="bold")
        ax.tick_params(colors=COLORS["text2"], labelsize=FONT_PLOT_LABEL-2)
    ...
    def _reset_params(self):
        ...
        self.var_src_type.set("Равномерный")
    ...
    def _export_data(self):
        ...
        try:
            ...
            density = MATERIALS_DB.get(mat_name, {}).get('density', 'N/A')
            
            with open(os.path.join(folder_name, "parameters.txt"), "w", encoding="utf-8") as f:
            ...
            with open(os.path.join(folder_name, "recommendations.txt"), "w", encoding="utf-8") as f:
                f.write(recs)
                
            dTdt = np.gradient(res["T_cycle"], res["t_arr"])
            with open(os.path.join(folder_name, "thermal_cycle.csv"), "w", newline="", encoding="utf-8") as f:
            ...
            for fig_attr, filename in plots_to_save:
                if hasattr(self, fig_attr):
                    ...
                    try:
                        fig.canvas.draw()
                        fig.savefig(os.path.join(folder_name, filename), dpi=300, bbox_inches='tight', facecolor=fig.get_facecolor())
                    except Exception as e:
